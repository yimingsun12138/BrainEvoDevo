...
# load RNA scope data -----------------------------------------------------
RNA_Scope_stat <- './res/step_129_fig_230814/ITGA2.xlsx'
...
table(mouse_DAPI$scale == mouse_ITGA2$scale)
table(mouse_DAPI$region == mouse_ITGA2$region)
table(mouse_DAPI$scale == mouse_PAX6$scale)
table(mouse_DAPI$region == mouse_PAX6$region)
...
mouse_DAPI$species <- 'mouse'
mouse_DAPI$ITGA2 <- mouse_ITGA2$IntDen / mouse_DAPI$IntDen
mouse_DAPI$PAX6 <- mouse_PAX6$IntDen / mouse_DAPI$IntDen

temp <- rbind(human_DAPI,monkey_DAPI,mouse_DAPI)

bar_table <- base::do.call(what = rbind,args = base::lapply(X = c('human','monkey','mouse'),FUN = function(x){
...
pdf(file = './res/step_129_fig_230814/ITGA2_normalized_fluorescence_intensity_barplot.pdf',width = 4.5,height = 4)
...
RNA_Scope_stat <- './res/step_129_fig_230814/GPX3.xlsx'
sheet_names <- excel_sheets(path = RNA_Scope_stat)
for (i in sheet_names) {
  temp <- i
  temp <- read_excel(path = RNA_Scope_stat,sheet = temp)
...
table(mouse_DAPI$scale == mouse_GPX3$scale)
table(mouse_DAPI$region == mouse_GPX3$region)
table(mouse_DAPI$scale == mouse_PAX6$scale)
table(mouse_DAPI$region == mouse_PAX6$region)
...
mouse_DAPI$species <- 'mouse'
mouse_DAPI$GPX3 <- mouse_GPX3$IntDen / mouse_DAPI$IntDen
mouse_DAPI$PAX6 <- mouse_PAX6$IntDen / mouse_DAPI$IntDen
...
#merge plot
p1 <- p1 + coord_cartesian(ylim = c(0,0.4))
p2 <- p2 + coord_cartesian(ylim = c(0,0.4)) + 
  theme(axis.title.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.text.y = element_blank())

pdf(file = './res/step_129_fig_230814/ITGA2_GPX3_normalized_fluorescence_intensity.pdf',width = 8,height = 4)
