...
## Script Purpose: macaque multiome RNA quality control                            ##
## Data: 2022.10.13                                                                ##
## Author: Yiming Sun                                                              ##
#####################################################################################
...
macaque_multiome_Seurat[['percent.mt']] <- PercentageFeatureSet(object = macaque_multiome_Seurat,features = MT_gene,assay = 'RNA')
...
meta_data$donor <- factor(meta_data$donor,levels = c('A50A','A50B','A82A','A82B'))
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_QC_vlnplot.pdf',width = 4,height = 6)
p1+p2+p3+plot_layout(ncol = 1)
dev.off()


# vlnplot of all macaque data for basic QC --------------------------------
#load data
macaque_multiome_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_multiome_Seurat_220802.rds')
...
p1 <- ggplot(meta_data,aes(x = donor,y = nCount_RNA,fill = donor,color = donor)) + 
  ...
  theme(legend.position = 'none',
        ...
        axis.line = element_line(color = 'black',size = 1)) + 
  ylab('# Reads') + labs(title = 'Multiome')

p3 <- ggplot(meta_data,aes(x = donor,y = nFeature_RNA,fill = donor,color = donor)) + 
  geom_violin() + 
  geom_boxplot(color = 'black',fill = 'white',width = 0.1,outlier.alpha = 0) + 
  ylim(c(0,6000)) + 
  scale_fill_manual(values = col_value[c('A50A','A50B','A82A','A82B')]) + 
...
p5 <- ggplot(meta_data,aes(x = donor,y = percent.mt,fill = donor,color = donor)) + 
  geom_violin() + 
  geom_boxplot(color = 'black',fill = 'white',width = 0.1,outlier.alpha = 0) + 
  scale_fill_manual(values = col_value[c('A50A','A50B','A82A','A82B')]) + 
  scale_color_manual(values = col_value[c('A50A','A50B','A82A','A82B')]) + 
...
p2 <- ggplot(meta_data,aes(x = donor,y = nCount_RNA,fill = donor,color = donor)) + 
  geom_violin() + 
  geom_boxplot(color = 'black',fill = 'white',width = 0.1,outlier.alpha = 0) + 
...
p4 <- ggplot(meta_data,aes(x = donor,y = nFeature_RNA,fill = donor,color = donor)) + 
  geom_violin() + 
  geom_boxplot(color = 'black',fill = 'white',width = 0.1,outlier.alpha = 0) + 
  ylim(c(0,6000)) + 
  scale_fill_manual(values = col_value[c('A50A','A82B','A84B','A84C','A68A','A68B')]) + 
...
p6 <- ggplot(meta_data,aes(x = donor,y = percent.mt,fill = donor,color = donor)) + 
  geom_violin() + 
  geom_boxplot(color = 'black',fill = 'white',width = 0.1,outlier.alpha = 0) + 
  scale_fill_manual(values = col_value[c('A50A','A82B','A84B','A84C','A68A','A68B')]) + 
  scale_color_manual(values = col_value[c('A50A','A82B','A84B','A84C','A68A','A68B')]) + 
...
p2+p4+p6+plot_layout(ncol = 1)

#all plot
pdf(file = './res/step_65_fig_221013/macaque_all_data_QC_vlnplot.pdf',width = 10,height = 6)
...
#merge to bulk data
table(rownames(macaque_multiome_Seurat) == rownames(macaque_RNA_Seurat))
RNA_expression <- macaque_RNA_Seurat@assays$RNA@counts
RNA_expression <- log1p(rowSums(RNA_expression)/sum(rowSums(RNA_expression))*1000000)

multiome_expression <- macaque_multiome_Seurat@assays$RNA@counts
multiome_expression <- log1p(rowSums(multiome_expression)/sum(rowSums(multiome_expression))*1000000)

#plot
pdf(file = './res/step_65_fig_221013/macaque_RNA_multiome_data_cor_scatter_plot.pdf',width = 4,height = 4)
ggplot(data = data.frame(RNA = RNA_expression,multiome = multiome_expression),aes(x = RNA,y = multiome)) + 
  ...
  xlab('RNA') + ylab('Multiome')
dev.off()

#save png
png(filename = './res/step_65_fig_221013/macaque_RNA_multiome_data_cor_scatter_plot.png',width = 350,height = 350)
ggplot(data = data.frame(RNA = RNA_expression,multiome = multiome_expression),aes(x = RNA,y = multiome)) + 
  geom_point(size = 0.1,alpha = 0.8,color = '#440154FF') + 
  xlim(c(0,7)) + ylim(c(0,7)) + 
...
gene_list <- rownames(RNA_expression)
RNA_expression <- base::do.call(what = cbind,args = base::lapply(X = donor_list,FUN = function(x){
  temp <- RNA_expression[,macaque_RNA_Seurat$donor == x]
  temp <- log1p(rowSums(temp)/sum(rowSums(temp))*1000000)
  return(temp)
}))
rownames(RNA_expression) <- gene_list
colnames(RNA_expression) <- donor_list

multiome_expression <- macaque_multiome_Seurat@assays$RNA@counts
donor_list <- c('A50A','A50B','A82A','A82B')
gene_list <- rownames(multiome_expression)
multiome_expression <- base::do.call(what = cbind,args = base::lapply(X = donor_list,FUN = function(x){
  temp <- multiome_expression[,macaque_multiome_Seurat$donor == x]
  temp <- log1p(rowSums(temp)/sum(rowSums(temp))*1000000)
  return(temp)
}))

rownames(multiome_expression) <- gene_list
colnames(multiome_expression) <- donor_list

#merge
colnames(RNA_expression) <- paste('RNA',colnames(RNA_expression),sep = '_')
colnames(multiome_expression) <- paste('Multiome',colnames(multiome_expression),sep = '_')
table(rownames(RNA_expression) == rownames(multiome_expression))

express_matrix <- cbind(multiome_expression,RNA_expression)

#cor matrix
cor_matrix <- cor(x = express_matrix,method = 'pearson')

#add annotation
tech_list <- unlist(base::lapply(X = colnames(cor_matrix),FUN = function(x){
  temp <- strsplit(x = x,split = '_',fixed = TRUE)
  return(temp[[1]][1])
}))
donor_list <- unlist(base::lapply(X = colnames(cor_matrix),FUN = function(x){
  temp <- strsplit(x = x,split = '_',fixed = TRUE)
  return(temp[[1]][2])
}))

col_anno <- HeatmapAnnotation(tech = tech_list,donor = donor_list,
                              col = list(tech = c('Multiome' = '#F47E1F','RNA' = '#FCBF6E'),
                                         donor = col_param$sample),
                              show_annotation_name = FALSE,which = 'column')
row_anno <- HeatmapAnnotation(tech = tech_list,donor = donor_list,
                              col = list(tech = c('Multiome' = '#F47E1F','RNA' = '#FCBF6E'),
                                         donor = col_param$sample),
                              show_annotation_name = FALSE,which = 'row')
col_fun <- colorRamp2(breaks = c(0.94,1),colors = c('white','red'))

#plot
p1 <- Heatmap(matrix = cor_matrix,show_row_names = TRUE,show_column_names = TRUE,
              cluster_rows = cluster_within_group(mat = cor_matrix,factor = c(tech_list)),
              cluster_columns = cluster_within_group(mat = cor_matrix,factor = c(tech_list)),
...
pdf(file = './res/step_65_fig_221013/macaque_all_data_pseudobulk_by_sample_cor_heatmap.pdf',width = 10,height = 10)
print(p1)
print(p2)
dev.off()
...
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_dimplot.pdf',width = 8,height = 8)
p1
p2
dev.off()

png(filename = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_dimplot.png',width = 500,height = 500)
p3
dev.off()
...
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_donor_dimplot.pdf',width = 8,height = 8)
p1
dev.off()

png(filename = './res/step_65_fig_221013/macaque_RNA_Seurat_donor_dimplot.png',width = 500,height = 500)
p2
dev.off()

# macaque multiome RNA UMAP plot ---------------------------------------------
#load data
macaque_multiome_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_multiome_Seurat_220802.rds')
...
#coordinate
x_lower <- min(macaque_multiome_Seurat@reductions$umap@cell.embeddings[,1])
x_upper <- max(macaque_multiome_Seurat@reductions$umap@cell.embeddings[,1])
y_lower <- min(macaque_multiome_Seurat@reductions$umap@cell.embeddings[,2])
y_upper <- max(macaque_multiome_Seurat@reductions$umap@cell.embeddings[,2])

#dimplot
p1 <- DimPlot(object = macaque_multiome_Seurat,group.by = 'cell_type',label = TRUE,repel = TRUE) + 
  little_axis(x_range = c(x_lower,x_upper),y_range = c(y_lower,y_upper),
              ratio = 0.2,margin_value = 1,x_label = 'UMAP_1',y_label = 'UMAP_2') + 
  theme_cowplot() + 
  NoAxes() + 
...
p2 <- DimPlot(object = macaque_multiome_Seurat,group.by = 'cell_type',label = FALSE) + 
  little_axis(x_range = c(x_lower,x_upper),y_range = c(y_lower,y_upper),
              ratio = 0.2,margin_value = 1,x_label = 'UMAP_1',y_label = 'UMAP_2') + 
  theme_cowplot() + 
  NoAxes() + 
...
p3 <- DimPlot(object = macaque_multiome_Seurat,group.by = 'cell_type',label = FALSE) + 
  theme_cowplot() + 
  NoAxes() + NoLegend() + 
  theme(aspect.ratio = 1,
        plot.title = element_blank()) + 
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_dimplot.pdf',width = 8,height = 8)
p1
p2
dev.off()

png(filename = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_dimplot.png',width = 500,height = 500)
p3
dev.off()
...
donor_seq <- c('A50A','A50B','A82A','A82B')
...
p1 <- DimPlot(object = macaque_multiome_Seurat,group.by = 'donor',label = FALSE) + 
  little_axis(x_range = c(x_lower,x_upper),y_range = c(y_lower,y_upper),
              ratio = 0.2,margin_value = 1,x_label = 'UMAP_1',y_label = 'UMAP_2') + 
  theme_cowplot() + 
  NoAxes() + 
...
p2 <- DimPlot(object = macaque_multiome_Seurat,group.by = 'donor',label = FALSE) + 
  theme_cowplot() + 
  NoAxes() + NoLegend() + 
  theme(aspect.ratio = 1,
        plot.title = element_blank()) + 
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_donor_dimplot.pdf',width = 8,height = 8)
p1
dev.off()

png(filename = './res/step_65_fig_221013/macaque_multiome_Seurat_donor_dimplot.png',width = 500,height = 500)
p2
dev.off()
...
#plot
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_marker_dotplot.pdf',width = 13,height = 6)
...
#plot
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_marker_dotplot.pdf',width = 13,height = 6)
my_dotplot(data_plot = dotplot_matrix,col.max = 2.5, col.min = -2.5,
           cols = c('#3B4992FF','white','#EE0000FF'),return_data_plot = FALSE) + 
  theme(axis.text.x = element_text(angle = 45,vjust = 1,hjust = 1),
        panel.border = element_rect(fill = NA,colour = 'black',size = 0.5),
...
# macaque RNA data marker gene list heatmap -------------------------------
...
#plot
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_marker_list_heatmap.pdf',width = 10,height = 10)
...
#lfc 0.3 pct 0.2 overlap 0.75
marker_gene_list <- base::lapply(X = donor_seq,FUN = function(x){
  temp <- my_seurat_find_specific_marker(seu.obj = macaque_multiome_Seurat,assay = 'RNA',ident.1 = x,group.by = 'cell_type',min.express = 0,log2fc_thresholf = 0.3,pct.1 = 0.2,overlap_ratio = 0.75,workers = 8,features = NULL,future.globals.maxSize = 200*(1024^3))
  print(paste(x,'done!',sep = ' '))
  gc()
  return(temp)
})

saveRDS(object = marker_gene_list,file = './res/step_65_fig_221013/cell_type_specific_marker_gene_lfc_0.3_pct_0.2_overlap_0.75.rds')

#lfc 0.25 pct 0.1 overlap 0.75
marker_gene_list <- base::lapply(X = donor_seq,FUN = function(x){
  temp <- my_seurat_find_specific_marker(seu.obj = macaque_multiome_Seurat,assay = 'RNA',ident.1 = x,group.by = 'cell_type',min.express = 0,log2fc_thresholf = 0.25,pct.1 = 0.1,overlap_ratio = 0.75,workers = 8,features = NULL,future.globals.maxSize = 200*(1024^3))
  print(paste(x,'done!',sep = ' '))
  gc()
  return(temp)
})

saveRDS(object = marker_gene_list,file = './res/step_65_fig_221013/cell_type_specific_marker_gene_lfc_0.25_pct_0.1_overlap_0.75.rds')

#lfc 0.5 pct 0.4 overlap 0.8
marker_gene_list <- base::lapply(X = donor_seq,FUN = function(x){
  temp <- my_seurat_find_specific_marker(seu.obj = macaque_multiome_Seurat,assay = 'RNA',ident.1 = x,group.by = 'cell_type',min.express = 0,log2fc_thresholf = 0.5,pct.1 = 0.4,overlap_ratio = 0.8,workers = 8,features = NULL,future.globals.maxSize = 200*(1024^3))
  print(paste(x,'done!',sep = ' '))
  gc()
  return(temp)
})

saveRDS(object = marker_gene_list,file = './res/step_65_fig_221013/cell_type_specific_marker_gene_lfc_0.5_pct_0.4_overlap_0.8.rds')

# macaque multiome data marker gene list heatmap --------------------------
#load data
macaque_multiome_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_multiome_Seurat_220802.rds')
donor_seq <- c('RG-1','RG-2','Cycling','IP','Ex-1','Ex-2','Ex-3','Ex-4','InMGE','InCGE','OPC','End','Per','VLMC','Mic')

#check marker
marker_gene_list <- readRDS(file = './data/parameter/specific_marker_list/221014/macaque_multiome/cell_type_specific_marker_gene_lfc_0.3_pct_0.2_overlap_0.75.rds')
marker_table <- base::do.call(what = rbind,args = base::lapply(X = names(marker_gene_list),FUN = function(x){
  temp <- data.frame(gene = marker_gene_list[[x]],cluster = x)
  rownames(temp) <- temp$gene
  other_cell_type <- donor_seq[donor_seq != x]
  marker_test <- my_seurat_marker_wilcox_test(seu.obj = macaque_multiome_Seurat,assay = 'RNA',ident.1 = x,ident.2 = other_cell_type,group.by = 'cell_type',min.express = 0,log2fc_thresholf = 0,pct.1 = 0,features = temp$gene,only.pos = TRUE,workers = 1,future.globals.maxSize = 2200*(1024^3))
  marker_test <- marker_test[rownames(temp),]
  temp <- cbind(temp,marker_test)
  print(paste(x,'done!',sep = ' '))
  return(temp)
...
cell_list <- base::lapply(X = donor_seq,FUN = function(x){
  temp <- colnames(macaque_multiome_Seurat)[macaque_multiome_Seurat$cell_type == x]
  if(length(temp) < 200){
    temp <- sample(x = temp,size = 200,replace = TRUE)
  }else{
    temp <- sample(x = temp,size = 200,replace = FALSE)
...
cell_list <- unlist(cell_list)
names(cell_list) <- macaque_multiome_Seurat@meta.data[cell_list,"cell_type"]

#express matrix
express_matrix <- macaque_multiome_Seurat@assays$RNA@data[marker_table_subset$gene,cell_list]
express_matrix <- t(scale(x = t(express_matrix)))
...
#plot
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_marker_list_heatmap.pdf',width = 10,height = 10)
Heatmap(matrix = express_matrix,cluster_rows = FALSE,cluster_columns = FALSE,show_column_names = FALSE,show_row_names = FALSE,
        top_annotation = col_anno,col = col_fun,use_raster = TRUE,column_split = factor(names(cell_list),levels = donor_seq),
        row_split = factor(marker_table_subset$cluster,levels = donor_seq),right_annotation = row_anno,name = 'z-score',
        height = unit(8,'inches'),width = unit(5.5,'inches'))
...
# macaque RNA featureplot -------------------------------------------------
#load data
macaque_RNA_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_RNA_Seurat_220803.rds')
marker_gene_list <- readRDS(file = './data/parameter/specific_marker_list/221014/macaque_RNA/cell_type_specific_marker_gene_lfc_0.3_pct_0.2_overlap_0.75.rds')

#feature plot legend
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_marker_featureplot_legend.pdf',width = 8,height = 7)
...
for (i in marker_gene_list) {
  char <- paste0('./res/step_65_fig_221013/RNA_feature_plot/macaque_RNA_Seurat_',i,'_featureplot_ordered.png')
...
#with order
for (i in marker_gene_list) {
  char <- paste0('./res/step_65_fig_221013/RNA_feature_plot/macaque_RNA_Seurat_',i,'_featureplot_without_ordered.png')
...
# macaque multiome featureplot -------------------------------------------------
#load data
macaque_multiome_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_multiome_Seurat_220802.rds')
marker_gene_list <- readRDS(file = './data/parameter/specific_marker_list/221014/macaque_multiome/cell_type_specific_marker_gene_lfc_0.3_pct_0.2_overlap_0.75.rds')

#feature plot legend
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_marker_featureplot_legend.pdf',width = 8,height = 7)
FeaturePlot(features = 'PAX6',order = TRUE,object = macaque_multiome_Seurat,cols = c('#e0ecf4','#8c6bb1','#4d004b'),pt.size = 0.1,max.cutoff = 'q99',slot = 'data') + 
  theme(aspect.ratio = 1) + 
  NoAxes() + 
  theme(plot.title = element_text(face = 'italic'))
dev.off()
...
marker_gene_list <- append(marker_gene_list,temp)
marker_gene_list <- unique(marker_gene_list)
marker_gene_list <- marker_gene_list[marker_gene_list %in% rownames(macaque_multiome_Seurat@assays$RNA@data)]

#with order
for (i in marker_gene_list) {
  char <- paste0('./res/step_65_fig_221013/multiome_feature_plot/macaque_multiome_Seurat_',i,'_featureplot_ordered.png')
  p <- FeaturePlot(features = i,order = TRUE,object = macaque_multiome_Seurat,cols = c('#e0ecf4','#8c6bb1','#4d004b'),pt.size = 0.001,max.cutoff = 'q99',slot = 'data') + 
    NoAxes() + 
    theme(plot.title = element_blank(),
          aspect.ratio = 1,legend.position = 'none')
  png(filename = char,width = 500,height = 500)
...
#with order
for (i in marker_gene_list) {
  char <- paste0('./res/step_65_fig_221013/multiome_feature_plot/macaque_multiome_Seurat_',i,'_featureplot_without_ordered.png')
  p <- FeaturePlot(features = i,order = FALSE,object = macaque_multiome_Seurat,cols = c('#e0ecf4','#8c6bb1','#4d004b'),pt.size = 0.001,max.cutoff = 'q99',slot = 'data') + 
    NoAxes() + 
    theme(plot.title = element_blank(),
          aspect.ratio = 1,legend.position = 'none')
  png(filename = char,width = 500,height = 500)
...
# macaque RNA donor and age distribution ----------------------------------------------
#load data
macaque_RNA_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_RNA_Seurat_220803.rds')
...
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_donor_proportion_barplot.pdf',width = 4,height = 4.5)
p
dev.off()
...
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_Age_proportion_barplot.pdf',width = 4,height = 4.5)
p
dev.off()
...
pdf(file = './res/step_65_fig_221013/macaque_RNA_Seurat_cell_type_proportion_by_Age_barplot.pdf',width = 5,height = 3.5)
p
dev.off()

# macaque multiome donor and age distribution ----------------------------------------------
#load data
macaque_multiome_Seurat <- readRDS(file = './processed_data/221008_summary/macaque_multiome_Seurat_220802.rds')
donor_proportion_matrix <- My_Cell_Proportion(meta_data = macaque_multiome_Seurat@meta.data,group.by = 'donor',split.by = 'cell_type')
donor_list <- c('A50A','A50B','A82A','A82B')
cell_type_list <- c('RG-1','RG-2','Cycling','IP','Ex-1','Ex-2','Ex-3','Ex-4','InMGE','InCGE','OPC','End','Per','VLMC','Mic')
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_donor_proportion_barplot.pdf',width = 4,height = 4.5)
p
dev.off()

#age proportion A50A/A50B (E80), A82A/A82B (E90), A84B/A84C (E84), A68A/A68B (E92)
macaque_multiome_Seurat$Age <- NA
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A50A','A50B'),"Age"] <- 'E80'
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A82A','A82B'),"Age"] <- 'E90'
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A84B','A84C'),"Age"] <- 'E84'
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A68A','A68B'),"Age"] <- 'E92'
age_proportion_matrix <- My_Cell_Proportion(meta_data = macaque_multiome_Seurat@meta.data,group.by = 'Age',split.by = 'cell_type')
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_Age_proportion_barplot.pdf',width = 4,height = 4.5)
p
dev.off()
...
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A84B','A84C'),"Age"] <- 'E84'
macaque_multiome_Seurat@meta.data[macaque_multiome_Seurat$donor %in% c('A68A','A68B'),"Age"] <- 'E92'
age_proportion_matrix <- My_Cell_Proportion(meta_data = macaque_multiome_Seurat@meta.data,group.by = 'cell_type',split.by = 'Age')
...
pdf(file = './res/step_65_fig_221013/macaque_multiome_Seurat_cell_type_proportion_by_Age_barplot.pdf',width = 5,height = 2.7)
p
dev.off()
