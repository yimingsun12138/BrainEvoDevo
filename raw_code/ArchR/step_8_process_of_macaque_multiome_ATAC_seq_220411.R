...
## Script Purpose: process of macaque multiome ATAC_seq                            ##
## Data: 2022.04.11                                                                ##
## Author: Yiming Sun                                                              ##
#####################################################################################
...
macaque_multiome_ArchR <- ArchRProject(
  ...
  outputDirectory = './processed_data/macaque_multiome_ArchR_220411',
  copyArrows = TRUE,
...
#filter cells
temp <- readRDS(file = './processed_data/macaque_multiome_ArchR_220307/Save-ArchR-Project.rds')
table(rownames(temp@cellColData) %in% rownames(macaque_multiome_ArchR@cellColData))
macaque_multiome_ArchR <- macaque_multiome_ArchR[rownames(macaque_multiome_ArchR@cellColData) %in% rownames(temp@cellColData)]

#save data
saveArchRProject(ArchRProj = macaque_multiome_ArchR, outputDirectory = './processed_data/macaque_multiome_ArchR_220411/', load = FALSE, overwrite = TRUE)
...
pdf(file = './res/step_8_fig_220411/TSS_enrichment_vs_nFrag_dotplot_after_filter.pdf',width = 16,height = 5)
...
pdf(file = './res/step_8_fig_220411/TSS_enrichment_vlnplot_after_filter.pdf',width = 5,height = 5)
p
dev.off()
...
pdf(file = './res/step_8_fig_220411/nFrag_vlnplot_after_filter.pdf',width = 5,height = 5)
p
dev.off()
...
pdf(file = './res/step_8_fig_220411/frag_length_after_filter.pdf',width = 7,height = 5)
...
pdf(file = './res/step_8_fig_220411/TSS_accessibility_by_sample_after_filter.pdf',width = 7,height = 5)
p + theme(legend.text = element_text(size = 10), legend.position = 'right')
dev.off()
...
# label transfer ----------------------------------------------------------
#load data
macaque_multiome_ArchR <- readRDS(file = './processed_data/macaque_multiome_ArchR_220411/Save-ArchR-Project.rds')
macaque_integration_Seurat <- readRDS(file = '../processed_data/220305_summary/macaque_integration_Seurat_220307.rds')
macaque_integration_Seurat <- macaque_integration_Seurat[,macaque_integration_Seurat$tech == 'scRNA']
...
temp <- macaque_integration_Seurat
...
macaque_integration_Seurat <- CreateSeuratObject(counts = macaque_integration_Seurat,project = 'macaque',assay = 'RNA',
...
#cluster C20 Cycling
#cluster C19 IP
#cluster C2 C3 C4 C5 C6 C7 C8 Ex
#Cluster C9 C10 InCGE
#Cluster C11 C12 C13 C14 InMGE

group_list <- SimpleList(
  none_neuron = SimpleList(
    RNA = colnames(macaque_integration_Seurat)[macaque_integration_Seurat$cell_type %in% c('End','Mic','OPC','Per')],
    ATAC = rownames(macaque_multiome_ArchR@cellColData)[macaque_multiome_ArchR$Clusters %in% c('C1','C15','C16')]
  ),
  npc = SimpleList(
    RNA = colnames(macaque_integration_Seurat)[macaque_integration_Seurat$cell_type %in% c('Cyc-G2M','Cyc-S','IP','RG-1','RG-2','RG-3')],
    ATAC = rownames(macaque_multiome_ArchR@cellColData)[macaque_multiome_ArchR$Clusters %in% c('C17','C18','C19','C20')]
  ),
  Ex_neuron = SimpleList(
    RNA = colnames(macaque_integration_Seurat)[macaque_integration_Seurat$cell_type %in% c('Ex-1','Ex-2','Ex-3')],
    ATAC = rownames(macaque_multiome_ArchR@cellColData)[macaque_multiome_ArchR$Clusters %in% c('C2','C3','C4','C5','C6','C7','C8')]
  ),
  In_neuron = SimpleList(
    RNA = colnames(macaque_integration_Seurat)[macaque_integration_Seurat$cell_type %in% c('InCGE','InMGE')],
    ATAC = rownames(macaque_multiome_ArchR@cellColData)[macaque_multiome_ArchR$Clusters %in% c('C9','C10','C11','C12','C13','C14')]
  )
)
...
macaque_multiome_ArchR <- addGeneIntegrationMatrix(
  ...
  reducedDims = "IterativeLSI",
  seRNA = macaque_integration_Seurat,
  addToArrow = TRUE, 
...
pdf(file = './res/step_8_fig_220411/macaque_multiome_RNA_cell_type_label_transfer_dimplot.pdf',width = 16,height = 7)
p1+p2+p3+plot_layout(ncol = 3)
dev.off()
...
temp$ori <- factor(temp$ori,levels = rev(c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','Cyc-S','Cyc-G2M','RG-3','RG-2','RG-1','OPC','Ependymal','Mic','End','Per')))
temp$prd <- factor(temp$prd,levels = rev(c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','Cyc-S','Cyc-G2M','RG-3','RG-2','RG-1','OPC','Mic','End','Per')))

pdf(file = './res/step_8_fig_220411/macaque_multiome_RNA_cell_type_label_transfer_confusion_heatmap.pdf',width = 4.5,height = 4.5)
ggplot(data = temp,aes(x=ori,y=prd,fill=Prob)) + 
  ...
  scale_fill_viridis() + 
  theme(aspect.ratio = 15/16,axis.title = element_text(size = 12,face = 'bold')) + 
  xlab('Gex_cell_type') + ylab('predict label')
dev.off()

#add Gex sub cell type
temp <- readRDS(file = '../res/step_22_fig_220330/macaque_multiome_Seurat_meta_data.rds')
macaque_RNA_seurat$sub_cell_type <- temp[colnames(macaque_RNA_seurat),"sub_cell_type"]
DimPlot(macaque_RNA_seurat,group.by = 'sub_cell_type',label = TRUE,repel = TRUE,reduction = 'harmonyUMAP')
macaque_multiome_ArchR$Gex_sub_cell_type <- macaque_RNA_seurat@meta.data[rownames(macaque_multiome_ArchR@cellColData),"sub_cell_type"]
...
cell_list_col <- c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','Cyc-S','Cyc-G2M','RG-3','RG-2','RG-1','OPC','Mic','End','Per')
cell_list_row <- c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','Cyc-S','Cyc-G2M','RG-3','RG-2','RG-1','Ependymal','OPC','Mic','End','Per')

pdf(file = './res/step_8_fig_220411/macaque_multiome_RNA_prediction_expression_cor_on_variable_gene.pdf',width = 6,height = 6)
Heatmap(matrix = temp[cell_list_row,cell_list_col],cluster_rows = FALSE,cluster_columns = FALSE,row_title = 'multiome express',column_title = 'prediction express',
        height = unit(4.27,'inches'),width = unit(4,'inches'),
        row_split = factor(c('In','In','Ex','Ex','Ex','Ex','NPC','NPC','NPC','NPC','NPC','NN','NN','NN','NN','NN'),levels = c('In','Ex','NPC','NN')),
        column_split = factor(c('In','In','Ex','Ex','Ex','Ex','NPC','NPC','NPC','NPC','NPC','NN','NN','NN','NN'),levels = c('In','Ex','NPC','NN')),
        border = TRUE,name = 'Cor')
dev.off()

#re-name cell type
macaque_multiome_ArchR$cell_type <- as.character(macaque_multiome_ArchR$Gex_cell_type)
macaque_multiome_ArchR@cellColData[macaque_multiome_ArchR$Gex_cell_type %in% c('End','Per'),"cell_type"] <- 'End/Per'
macaque_multiome_ArchR@cellColData[macaque_multiome_ArchR$Gex_cell_type %in% c('Cyc-S','Cyc-G2M'),"cell_type"] <- 'Cycling'
macaque_multiome_ArchR@cellColData[macaque_multiome_ArchR$Gex_cell_type %in% c('RG-1','RG-2','RG-3'),"cell_type"] <- 'RG'
...
#Ependymal not speific, understandable
...
markerList <- markerList[c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','RG','OPC')]
...
pdf(file = './res/step_8_fig_220411/macaque_multiome_ArchR_marker_peak_heatmap.pdf',width = 12,height = 6)
Heatmap(matrix = express_matrix,cluster_columns = FALSE,cluster_rows = FALSE,show_row_names = TRUE,show_column_names = FALSE,
        ...
        column_split = factor(names(feature_list),levels = c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','RG','OPC')),
        border = TRUE,row_split = factor(c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','RG','OPC'),
                                         levels = c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','RG','OPC')),
        row_title = 'Pseudo bulk sample',column_title = 'marker peak')
dev.off()
...
p <- plotBrowserTrack(
  ...
  useGroups = c('InCGE','InMGE','Ex-3','Ex-2','Ex-1','IP','RG','OPC')
)
